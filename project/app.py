...
def sort_events_by_date(events, order):
    events_with_dt = [
        (event, datetime.strptime(f"{event.date} {event.time}", "%Y-%m-%d %H:%M"))
        for event in events
    ]
...
def sort_events_by_name(events, order):
    if order == "A to Z":
        ...
        sorted_events = sorted(
            events, key=lambda event: event.name.lower(), reverse=True
        )
    ...
    if request.method == "POST":
        if request.form.get("remove-from-bookmarks") != None:
            ...
            if event_to_remove in user.bookmarked_events:
                ...
                error_msg = (
                    str(event_to_remove)
                    + "is not associated with this user's bookmarked events"
                )
...
def handle_button_click():
    ...
    return jsonify(
        success=True, response_message=response_message, added=is_bookmarked
...
def filter_events(searching=False):
    ...
    error_msg = ""
    if (
        user.event_types_checked == None
        or user.event_types_checked == "[false]"
        or user.event_types_checked == "[]"
    ):
        ...
        for filter in event_types_checked:
            filtered_events = filtered_events + (
                Event.query.filter(Event.event_type.contains(filter)).all()
            )
    ...
    if request.method == "POST":
        if not searching:
            if (
                request.form.getlist("filter") != []
                and request.form.getlist("reset-filters") == []
            ):
...
def main_dashboard():
    ...
    if "view_event_details" in session:
        ...
        attendee_record = Attendee.query.filter_by(
        ...
        user_rating_value = (
            user_rating.rating if user_rating else 0
        )  # Base case will be 0
    ...
    if request.method == "POST":
        ...
        if request.form.get("event-details") != None:
            ...
            if (
                ...
                and attendee_record.notification_preference != -1
            ):
                notification_checked = True
    ...
    if session["show-bookmarked"] == "show-bookmarked":
        ...
        event_ids = [
            event.id for event in events if event.id not in bookmarked_evevnts_ids_set
        ]
...
def search_event():
    ...
    if keyword:
        results = Event.query.filter(
            Event.name.contains(keyword), Event.id.in_(filtered_events_ids)
        ).all()
...
def download_ics_file():
    ...
    preference = (
        int(request.form.get("preference")) if request.form.get("preference") else None
    )
    event = Event.query.filter_by(id=event_id).first()
    c = ics.Calendar()
    e = ics.Event()
    ...
    if action == "attend":
        ...
        if not attendee:
            new_attendee = Attendee(
                user_id=user.id, event_id=event.id, notification_preference="-1"
            )
...
def my_account_event_history(username):
    ...
    for attendee in attendee_records:
        ...
        event_datetime_dt = datetime.strptime(
...
def get_current_user_friends(username):
    ...
    if current_user:
        ...
        )  # This depends on how your user's friends are stored/retrieved
    else:
        return []
...
def my_account_friends(username):
    ...
    return render_template(
        "my_account_friends.html",  # Make sure the template name matches your setup
        username=username,
        interests=interests,
...
def my_account_notification():
    ...
    notif_events_with_prefs = [
        ...
        if attendee.notification_preference != -1
    ]

    if request.method == "POST":
        updated_event_id = request.form.get(
            "updated_event_id"
        )  # Make sure this matches your form input name
        updated_notification = request.form.get("updated_notification")
        attendee_record = Attendee.query.filter_by(
            user_id=user.id, event_id=updated_event_id
        ).first()
...
def my_account_edit_profile():
    ...
    if request.method == "POST":
        ...
        return render_template(
            ...
            interests=get_user_interests(),
            profile_picture=get_user_profile_picture(),
            profile_pics=profile_pic_types,
            top_right_image=None,
        )
...
def add_event():
    ...
    event_datetime_dt = datetime.strptime(event_datetime, "%Y-%m-%d %H:%M").replace(
    ...
    if request.method == "POST":
        if "finish_edit" in request.form:
            ...
            if event_datetime_dt > current_datetime:
                ...
                db.session.commit()
                return redirect(
                    url_for("my_account_myevents", username=session.get("username"))
                )
            else:
