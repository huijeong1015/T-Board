...
def dfs(graph, start, k):
    ...
    while stack:
        ...
        if depth <= k and vertex not in visited:
            visited.add(vertex)
            if depth > 0 and vertex not in graph[start]:  # Exclude start user and their direct friends
...
def show_users():
    ...
    user_list_html += '</ul>'
    return render_template_string(f"<h1>Users, Their Friends, and Events</h1>{user_list_html}")

@app.route('/recommendations')
def friend_recommendations():
    k = int(request.args.get('k', 2))
    users = User.query.all()
    user_data = {user.username: set(friend.username for friend in user.friends) for user in users}
    
    recommendations_html = '<ul>'
    for username in user_data:
        recommendations = dfs(user_data, username, k)
        recommendations = recommendations - user_data[username]  # Filter out existing friends
...
def new_events():
    ...
    event_data = {event.name: [attendee.username for attendee in event.attendees] for event in events}
    event_list_html = '<ul>'
    ...
    event_list_html += '</ul>'
    return render_template_string(f"<h1>Events and Their Attendees</h1>{event_list_html}")