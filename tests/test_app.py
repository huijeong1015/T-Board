...
def test_login_invalid_user(client):  # invalid username
    response = client.post(
        "/login",
        data=dict(username="'non_existent_user'", password="password"),
        follow_redirects=True,
    )
...
def test_register_existing_username(client):
    ...
    response = client.post(
        "/register", data={"input-id": "existing_user"}, follow_redirects=True
    )
    assert response.request.path == "/register/"
    ...
    with client:
        with app.app_context():
            ...
            test_user = User(
                ...
                email="someone1@mailutoronto.ca",
                interests="testing",
                profile_picture="Default",
            )
            test_event = Event(
...
def test_attend_event(client):
    with client:
        ...
        # Attend the event
        response = client.post(
            "/attend_event/1", data={"action": "attend"}, follow_redirects=True
        )
        assert response.status_code == 200
        # Unattend the event
        response = client.post(
            "/attend_event/1", data={"action": "unattend"}, follow_redirects=True
        )
        assert response.status_code == 200
...
def test_my_account_event_history(client):
    with client:
        test_user = User(
            username="test_user",
            password="test_password",
            email="test_user@mail.utoronto.ca",
            interests="testing",
            profile_picture="Default",
        )
        db.session.add(test_user)
        event_attended = Event(
            ...
            time="12:00",
            location="Test Location",
            description="Attended Event Description",
            event_type="Networking",
        )
...
def test_get_current_user_friends(client):
    with client:
        ...
        friend1 = User(
            username="friend1",
            password="friend1_password",
            email="friend1@mail.utoronto.ca",
            interests="friends",
            profile_picture="Default",
        )
        friend2 = User(
            username="friend2",
            password="friend2_password",
            email="friend2@mail.utoronto.ca",
            interests="friends",
            profile_picture="Default",
        )
        db.session.add(test_user)
...
def test_my_account_friends(client):
    with client:
        with app.app_context():
            ...
            app.config[
                "WTF_CSRF_ENABLED"
            ] = False  # Disable CSRF protection for testing
...
def test_edit_event(client):
    with client, app.app_context():
        ...
        response = client.post(
            "/edit_event/1",
            data={"input-name": "Updated Event Name"},
            follow_redirects=True,
        )
        assert response.status_code == 200
        # Delete the event
        response = client.post(
            "/edit_event/1",
            data={"delete_event": "Delete Event"},
            follow_redirects=True,
        )
        assert response.status_code == 200
...
def test_show_users(client):
    with client, app.app_context():
        ...
        user2 = User(
            username="user2",
            password="password",
            email="someone2@mailutoronto.ca",
            interests="testing",
            profile_picture="Default",
        )
        user3 = User(
            username="user3",
            password="password",
            email="someone3@mailutoronto.ca",
            interests="testing",
            profile_picture="Default",
        )
...
def test_are_you_sure(client):
    ...
    response = client.post(
        "/are_you_sure/1", data={"yes": "yes"}, follow_redirects=True
    )
    assert response.status_code == 200
...
def test_filter_friends_case_insensitive(client):
    with client:
        ...
        with app.test_request_context():
            ...
            non_existing_user_friends = filter_friends_by_search_term(
                friends_list, "non-existing"
            )
            assert non_existing_user_friends == []
...
def test_remove_friend(client):
    ...
    with client:
        with app.app_context():
            ...
            db.session.commit()
            client.post(
                "/login", data=dict(username="test_user1", password="test_password1")
            )
